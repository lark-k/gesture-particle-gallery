...
网站默认打开拾光首页 `/`。首页采用连续暖米白底色，完整相册在鼠标悬停时经过粒子散开播放彩色家庭回忆短视频，移开后聚拢恢复原图；触屏轻触体验，键盘聚焦后可用 Esc 还原。暂停动态或系统要求减少动态时保留完整原图，首次加载不播放缩放入场。

首页注册、登录后进入相册集 `/albums`，创建或选择相册后进入 `/albums/:id` 对应的沉浸式画廊、上传照片。未登录直接访问相册链接会先显示登录入口，登录后继续原相册。首页「先看看示例」和风景卡片进入 `/demo`，提供 25 张影像样例（20 张摄影与 5 张生成影像）。新账户从空相册列表开始；退出后返回网站首页。首页按需加载粒子渲染，画廊与手势模块仅在进入画廊时加载。

相册首页前三本采用一大两小布局，第四本起进入下方书本轮播。可编辑名称、简介和封面，选择六种内置素材或当前相册中的照片，并调整封面取景；编辑窗口的「放到首页首位」可调整展示顺序。相册支持名称搜索和删除，删除会在确认后同时移除其中照片。暖色窗影缓慢流动，鼠标悬停会掀开书封、露出内页；点击后通过翻书和推进动画进入画廊。手机支持左右滑动更多相册，动态效果遵循系统的减少动态偏好。

首次启动新版本会自动迁移 SQLite，将各账户旧照片和待完成上传归入各自的「默认相册集」，保留原照片 ID 与 OSS 对象路径。相册之间按归属隔离，上传配额仍按整个账户计算。接口、迁移与交互说明见 [相册集实现说明](docs/ALBUMS.md)。
