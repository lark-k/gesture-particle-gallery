...
| 操作 | 鼠标 / 触摸 / 按钮 | 手势 |
|---|---|---|
| 旋转 | 左右拖动，或两侧箭头 | 张掌横向扒拉，停稳后再次挥动 |
| 查看照片 | 点击可见照片；也可点击「帧记忆」打开目录 | 食指指向并停留至边缘亮起，捏合后向自己拉 |
| 返回总览 | 返回按钮 / Esc | 查看时张掌并向屏幕推 |
| 完整分解 / 聚合 | 查看状态的「分解为粒子 / 聚合照片」按钮 | 同样可用常规按钮 |

手向自己拉，通常是**远离位于屏幕附近的摄像头，掌部投影缩小**；向屏幕推则掌部投影增大。代码使用掌宽、腕至指根尺度、姿态比例、持续时间和捏合锁定；没有将手腕原点或掌心原点的 Z 当作绝对距离。

默认单手操作。张掌横扫后需停稳约 420 ms 再触发，防止回位变成反向挥动。捏合本身不会拉近。摄像头由按钮主动开启；关闭、退出或切换账户释放轨道。模型优先在 Worker 中推理，GPU 失败用 Worker CPU，Worker 不可用时使用主线程 12 Hz CPU 兼容模式。

右下角调节画质。帮助中可选择减少动态效果，并重试失败纹理；默认跟随系统减少动态偏好。右下角滑杆图标展开调试面板，显示候选、锁定目标、掌部尺度、动作进度、阈值、FPS、GPU 资源数量、旋转方向坐标与原位误差。
...
src/
  ...
  gesture/recognizer.ts       时间序列手势、平滑、阈值、滞回、冷却
...
| `GESTURE.dwellMs` | 指向停留 420 ms |
| `pinchEnter` / `pinchExit` | 掌尺度归一化后 0.30 / 0.46 |
| `pullRatio` / `pushRatio` | 掌部尺度 0.79 / 1.27 |
| `depthHoldMs` / `minDepthMs` | 阈值持续 170 ms，动作至少 300 ms |
| `cooldownMs` / `rearmStillMs` | 冷却 1,000 ms，停稳 420 ms |
